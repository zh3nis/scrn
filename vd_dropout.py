...
class MyDropoutWrapper(RNNCell):
  ...
  def __init__(self, cell, input_keep_prob=1.0, output_keep_prob=1.0,
    ...
    self._recurrent_context_noise = None
    ...
    if variational_recurrent:
      ...
      self._recurrent_output_noise = _enumerated_map_structure(
          ...
          cell.output_size)
      self._recurrent_context_noise = _enumerated_map_structure(
          lambda i, s: batch_noise(s, inner_seed=self._gen_seed("state", i)),
          cell.state_size.c)
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    if _should_dropout(self._state_keep_prob):
      ...
      new_state = tf.nn.rnn_cell.LSTMStateTuple(s, h)
      
      out_s = self._dropout(s, "state",
                            self._recurrent_context_noise,
                            self._output_keep_prob)
      output = tf.concat([out_s, h], axis=1)
    
    #if _should_dropout(self._output_keep_prob):
    #  if self._state_keep_prob == self._output_keep_prob:
    #    context_size = self._cell.state_size.c
    #    out_s, out_h = output[:, :context_size], output[:, context_size:]
    #    out_s = self._dropout(out_s, "state",
    #                           self._recurrent_context_noise,
    #                           self._state_keep_prob)
    #    out_h = self._dropout(out_h, "state",
    #                           self._recurrent_state_noise,
    #                           self._state_keep_prob)
    #    output = tf.concat([out_s, out_h], axis=1)
    #  else:
