...
import argparse
import copy
import vd_dropout as my_dropout
import collections
...
def parse_args():
  ...
  parser.add_argument('--vd', default='1',
...
class SCRNCell(tf.nn.rnn_cell.RNNCell):             
  ...
  def __call__(self, inputs, state, scope=None):
    with tf.variable_scope(scope or "scrn_cell"):
      ...
      new_s = tf.multiply(0.1, tf.matmul(inputs, self.B)) + tf.multiply(0.9, s)
...
class Model:
  ...
  def __init__(self, config, need_reuse=False):
    ...
    def my_cell(layer_num):
      if layer_num == 0:
        return SCRNCell(word_emb_dim, 
                        hidden_size, 
                        context_size)
      else:
        return SCRNCell(context_size + hidden_size, 
                        hidden_size, 
                        context_size)
    ...
    #                      if 'bias' not in v.name and 'Bias' not in v.name]) * config.weight_decay
